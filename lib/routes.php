...
function build_url($path, $arg_count, $args, $is_ssl_required=false)
{
  ...
  if (count($args) > $arg_count) wicked_error("Wrong number of arguments", array($path, $arg_count, $args));
  $parts = explode('?', $path);
  $extra_args = array();
  if(count($parts)>1)
  {
...
/*
  if(count($_SESSION['__wicked']['sessions'])>1)
  {
    $qs['__session_id'] = $_SESSION['__wicked']['current_session_id'];
  }
...
function map($event_name, $path, $routed_event_name = null, $url_generator_name=null, $is_ssl_required = false)
{
  ...
  $pattern = join("\\/",$parts);
//  $pattern = $__wicked['app_routing_prefix'] . $pattern;
  $pattern = "/^$pattern\$/";
