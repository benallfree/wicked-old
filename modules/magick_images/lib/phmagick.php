...
class phmagick{
        ...
        if(key_exists($name, $this->loadedPlugins)) {
        	if(! is_null($version)) {
        	   if( property_exists($this->loadedPlugins[$name], 'version') ){
	        	   	...
	        	   	wicked_error('Plugin "'.$name.'" version ='.$this->loadedPlugins[$name]->version . ' required >= ' . $version);
        	   }
        	}
        	return true ;
        }

      wicked_error('Plugin "'.$name.'" not found!');
    	return false;
    }
	...
	public function execute($cmd){
        ...
        if($ret != 0)
        {
          wicked_error("PHPMagick failed", array('cmd'=>$cmd, 'out'=>$out, 'ret'=>$ret));
        }
