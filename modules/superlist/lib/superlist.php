...
{
  global $__wicked;
  $config = $__wicked['modules']['superlist'];
  ...
  if(is_array($meta))
  {
    ...
    if(!isset($meta['controls'][$cmd]) || $meta['controls'][$cmd] !== 'custom')
    {
      ...
      echo template("superlist.$cmd", $meta);
...
function superlist_format($value, $field_info)
{
  ...
  switch($field_info['type'])
  {
    ...
    default:
      wicked_error("Unsupported type {$field_info['type']}");
  }
}
...
function superlist_build_meta($meta)
{
  ...
  {
    wicked_error("Namespace '{$meta['namespace']}' already being used by a superlist on this page.");
  }
  ...
  foreach($meta['list'] as $k=>$v)
  {
    ...
    {
      $v['type'] = 'text';
//      wicked_error("\$meta did not specify type information for $k and there is not enough information to infer a type from the data model.");
    }
  ...
  if($meta['controls']['add'])
  {
    ...
    {
      $new_object = eval("return new {$meta['klass']}();");
      $meta['new_object'] = $new_object;
    }
    
    if(isset($meta['add']))
    {
      $meta['new_object']->superform->sections = $meta['add'];
    } else {
      if(!$meta['new_object']->superform->sections)
      {
        $meta['new_object']->superform->sections = $meta['sections'];
      }
    }
  }
  ...
  if($meta['controls']['edit'] && p("{$meta['namespace']}[oid]"))
  {
    ...
    if(isset($meta['edit']))
    {
      $meta['edit_object']->superform->sections = $meta['edit'];
    } else {
      if(!$meta['edit_object']->superform->sections)
      {
        $meta['edit_object']->superform->sections = $meta['sections'];
      }
    }
  }
