...
function add_breadcrumb($name, $url)
{
  ...
  global $breadcrumbs;
  $keys = array_keys($breadcrumbs);
  if(count($keys)==0) wicked_error("Must use begin_breadcrumb() before add_breadcrumb().");
  $id = $keys[count($keys)-1];
...
function add_breadcrumb_button($label, $url)
{
  ...
  if(count($keys)==0) wicked_error("Must use begin_breadcrumb() before add_breadcrumb_button().");
  $id = $keys[count($keys)-1];
